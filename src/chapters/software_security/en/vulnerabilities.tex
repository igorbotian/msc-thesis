...
%
While formatted output functions, for instance \SourceCode{sprintf()} of C programming language, that wrute data to an array of characters are used, it is supposed that the buffer has intenionally big size \Reference{CERTCCSecureCodingStandard2007}. 
%
...
\begin{itemize}
	...
	\item utilization of automatic static analysis tools (RATS \WebSite{RATS}, Flawfinder \WebSite{Flawfinder}, PScan \WebSite{PScan}, and others) 
	\item utilization of additional libraries that increase safety (FormatGuard \WebSite{FormatGuard}, libformat \WebSite{Libformat}, libsafe \WebSite{Libsafe}, and others)
\end{itemize}
...
\Paragraph{Example}
%
In the listing below a fragment of code written in Java language represents \Abbreviation{API} for providing information about range of goods in an internet-store. 
%
...
%
Originally, after each checking of the resource availability (\SourceCode{isResourceAvailable()} method) it is possible to handle the resource data (\SourceCode{processResourceData()} method). 
%
